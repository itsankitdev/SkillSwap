...
import { getCategoryColor, getLevelColor, getInitials } from '../../utils/helpers';
...
const SkillCard = ({ skill, showActions = false, onDeleted }) => {
  const { _id, title, description, category, type, level, creditCost, user, tags, isLearned } = skill;
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 p-5 h-full flex flex-col gap-3">
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex gap-2 flex-wrap">
              ...
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${getLevelColor(level)}`}>
                {level}
              </span>
            </div>
          ...
          {tags?.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {tags.slice(0, 3).map((tag) => (
                <span key={tag} className="text-xs bg-gray-50 text-gray-500 px-2 py-0.5 rounded-md">
                  #{tag}
                </span>
              ))}
            </div>
